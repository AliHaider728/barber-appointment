...
import { Clock, Loader2, Sparkles, User } from 'lucide-react';
            ...
            {maleServices.length > 0 && (
              <div className="mb-16">
                <div className="text-center mb-10">
                  <h3 className="text-3xl lg:text-4xl font-black text-black uppercase tracking-tight inline-flex items-center gap-3">
                    <User className="w-8 h-8 text-blue-600" />
                    Male Services
                  </h3>
                  <p className="text-gray-600 mt-2">Expert grooming for men</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {maleServices.map((service, index) => (
                    <ServiceCard key={service._id} service={service} index={`male-${index}`} />
                  ))}
                </div>
              </div>
            )}
            ...
            {femaleServices.length > 0 && (
              <div className="mb-16">
                <div className="text-center mb-10">
                  ...
                  <p className="text-gray-600 mt-2">Styling & care for women</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {femaleServices.map((service, index) => (
                    <ServiceCard key={service._id} service={service} index={`female-${index}`} />
                  ))}
                </div>
              </div>
            )}
