...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      const mountWidth = mount.clientWidth || 1;
      const mountHeight = mount.clientHeight || 1;
      const pr = currentDprRef.current;

      // Adjust resolution based on footer's responsive height
      const footerHeight = mount.parentElement ? parseFloat(getComputedStyle(mount.parentElement).height) : 700;
      const aspectRatio = mountWidth / mountHeight;
      const targetHeight = footerHeight;
      const targetWidth = targetHeight * aspectRatio;

      renderer.setPixelRatio(pr);
      renderer.setSize(targetWidth, targetHeight, false);
      uniforms.iResolution.value.set(targetWidth * pr, targetHeight * pr, pr);
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRGB(color || '#D4AF37');
  ...
  return <div ref={mountRef} style={{ ...style, position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} />;
};
