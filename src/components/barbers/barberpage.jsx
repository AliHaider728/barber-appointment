    ...
    <div className="bg-gradient-to-br from-[#faf7f2] via-[#f5f1ea] to-[#faf7f2] min-h-screen" id="BarbersPage">
      <section className="max-w-7xl mx-auto px-4 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {barbers.length === 0 ? (
            <p className="col-span-full text-center text-gray-500 py-12">No barbers available.</p>
          ) : (
            barbers.map((barber) => (
              <div
                key={barber._id}
                className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-gray-100 hover:border-[#D4AF37]"
              >
                {/* Name */}
                ...
                <p className="text-base text-gray-700 mb-5">
                  {barber.branch?.name || 'Unknown Branch'}, {barber.branch?.city || ''}
                </p>
