    ...
    try {
      ...
      if (isLogin) {
        ...
        ({ data, error: supabaseError } = await supabase.auth.signUp({
          email,
          password,
          options: { data: { role: 'user' } },  // Default to user role for signups here; admins/barbers handled separately
        }));
      }
        ...
        <div className="bg-white p-6 sm:p-8 md:p-10 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100">
          ...
          <div className="mt-4">
            <button
              ...
              className="w-full bg-white text-gray-900 font-bold py-3.5 rounded-lg border-2 border-gray-300 hover:bg-gray-50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
              disabled={loading}
            >
